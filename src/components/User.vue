...
<template>
    <div class="py-2 px-2 m-1 inline-block rounded-md relative"
         :class="[ 
            props.user.gender=='女' ?'bg-red-300 border-red-400' : 'bg-blue-300 border-blue-400',
            check_on_court(props.user.id, 0) ?'border-4 border-yellow-600' : 'border-2',
         ]">
        ...
        <div class="flex align-center justify-between min-w-min">
            <span class="cursor-pointer border-b border-black" v-text="show_name" @click="select_user(user_index)"></span>
            <span class="ml-4">等待：<span v-text="user.wait"></span></span>
